...
class Gamer:
    def __init__(self, nodes, abs_infosets, roots, leaves, players, it, chance_sampling = False):
        ...
        self.cumulative_regrets = self.empty_strategies_init(self.root, abs_infosets)
        self.cumulative_sigma   = self.empty_strategies_init(self.root, abs_infosets)
        self.nash_equilibrium   = self.empty_strategies_init(self.root, abs_infosets)
...
## per ogni nodo calcola la somma dei payoff pesata sulla probabilità di arrivarci
    ...
    def exp_ut_recursive(self, node):
        ...
        for idaction in range(len(self.nodes.Actions[node])):
            value +=  self.nash_equilibrium[self.nodes.Abs_Map[node]][idaction] * self.exp_ut_recursive(self.nodes.Direct_Sons[node][idaction])
        return value
################################################################################
    ...
    def init_sigma(self,node, abs_infosets, output = None):
        ...
        for index, row in abs_infosets[abs_infosets.Dads != 999999].iterrows():
            try:
                ...
                output[index] = {action: row.Actions_Prob[action] for action in row.Actions}
        #print(output)
        return output

    def empty_strategies_init(self,node, abs_infosets, output = None):
        output = dict()
        for index, row in abs_infosets[abs_infosets.Dads != 999999].iterrows():
